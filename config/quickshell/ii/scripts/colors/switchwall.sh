...
pre_process() {
    ...
    if [[ "$mode_flag" == "dark" ]]; then
        ...
        gsettings set org.gnome.desktop.interface gtk-theme 'Gruvbox-Dark-Compact-Soft'
        gsettings set org.gnome.desktop.interface icon-theme 'Gruvbox-Plus-Dark'
        jq '.appearance.lightMode = false' "$SHELL_CONFIG_FILE" > "$SHELL_CONFIG_FILE.tmp" && mv "$SHELL_CONFIG_FILE.tmp" "$SHELL_CONFIG_FILE"
        kwriteconfig5 --file dolphinrc --group Colors --key ColorScheme GruvboxPlusDark
        kwriteconfig5 --file kdeglobals --group General --key ColorScheme GruvboxPlusDark
    elif [[ "$mode_flag" == "light" ]]; then
        gsettings set org.gnome.desktop.interface color-scheme 'prefer-light'
        gsettings set org.gnome.desktop.interface gtk-theme 'Gruvbox-Light-Compact-Soft'
        gsettings set org.gnome.desktop.interface icon-theme 'Gruvbox-Plus-Light'
        jq '.appearance.lightMode = true' "$SHELL_CONFIG_FILE" > "$SHELL_CONFIG_FILE.tmp" && mv "$SHELL_CONFIG_FILE.tmp" "$SHELL_CONFIG_FILE"
        kwriteconfig5 --file dolphinrc --group Colors --key ColorScheme GruvboxPlusLight
        kwriteconfig5 --file kdeglobals --group General --key ColorScheme GruvboxPlusLight
    fi
...
post_process() {
    echo "DEBUG: post_process called at $(date)" >> /tmp/switchwall-debug.log
...
switch() {
    echo "DEBUG: switch() called with imgpath=$1 mode=$2 at $(date)" >> /tmp/switchwall-debug.log
    imgpath="$1"
    mode_flag="$2"
    type_flag="$3"
    color_flag="$4"
    ...
    # Apply Gruvbox colors to override MaterialYou in kdeglobals
    # Apply the appropriate Gruvbox scheme based on current mode
    echo "DEBUG: lightMode = $(jq -r .appearance.lightMode "$SHELL_CONFIG_FILE")" >> /tmp/switchwall-debug.log
    if [[ "$(jq -r .appearance.lightMode "$SHELL_CONFIG_FILE")" == "true" ]]; then
        echo "DEBUG: Applying GruvboxPlusLight" >> /tmp/switchwall-debug.log
        ~/.config/quickshell/ii/scripts/colors/apply-gruvbox.sh GruvboxPlusLight
    else
        echo "DEBUG: Applying GruvboxPlusDark" >> /tmp/switchwall-debug.log
...
main() {
    echo "DEBUG: main() called with args: $@" >> /tmp/switchwall-debug.log
    imgpath=""
    mode_flag=""
    type_flag=""
    color_flag=""
